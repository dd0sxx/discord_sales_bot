...
async function main() {
  ...
  console.log(process.env.COLLECTION_SLUG, "https://api.opensea.io/api/v1/events?" + params)
  const openSeaResponse = await fetch(
    "https://api.opensea.io/api/v1/events?" + params).then((resp) => resp.json());
  
  // if (openSeaResponse.asset_events?.reverse() !== undefined || null) {
    console.log('recent sale: ', openSeaResponse?.asset_events?.reverse())
...
main()
  .then((res) =>{ 
    if (!res) console.log("No recent sales")
    process.exit(0)
  })
  .catch(error => {
    console.error(error);
